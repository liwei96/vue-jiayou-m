<template>
  <div class="talk">
    <h2>
      ...
      <!-- </router-link> -->
      <span>允家咨询师-{{staffname}}为您服务</span>
      <img class="morepeo" src="~/assets/talklist.png" alt @click="golist" />
      <i v-if="t3">1</i>
    </h2>
    <div class="con" id="Con" @click="prompttag = false">
      <p class="time">{{time}}</p>
      ...
      <div :class="item.className" v-for="(item,val) in cons" :key="val">
    ...
    <p class="tishi" v-if="t2">正在为您分配业务员，请稍候....</p>
  </div>
</template>
<script>
...
export default {
    ...
    init() {
      ...
      this.ws.onopen = function(event) {
        ...
        if (!localStorage.getItem("webkey")) {
          ...
          let ip = ip_arr["ip"];
          localStorage.setItem("webkey", key);
          let sign = ip + ":" + key;
          let actions = {
            //要发送的数据
            class: "Connect",
          ...
          if (data.code != 500) {
            if (data.action == "talk") {
              ...
              if (sid == data.content.id) {
                that.t2 = false;
                let img = sessionStorage.getItem("staff" + sid);
              ...
              if (
                data.content.indexOf(sessionStorage.getItem("sid")) != -1 &&
                sessionStorage.getItem("sid")
              ) {
                that.t2 = false;
              }
              if (sessionStorage.getItem(sessionStorage.getItem("projectid"))) {
                that.test(sessionStorage.getItem("sid"));
              }
              ...
              if (sessionStorage.getItem("sid") == data.content.sid) {
                that.t2 = false;
              }
              if (
                !sessionStorage.getItem(sessionStorage.getItem("projectid"))
            ...
            let key = localStorage.getItem("webkey", key);
            let ip = ip_arr["ip"];
            let sign = ip + ":" + key;
            let actions = {
              //要发送的数据
  ...
  mounted() {
    ...
    // if(!localStorage.getItem('connectws')){
  ...
  beforeDestroy() {
    ...
    if (sessionStorage.getItem("sid")) {
      ...
      request.onsuccess = function(e) {
        ...
        let time = sessionStorage.getItem("talktime");
